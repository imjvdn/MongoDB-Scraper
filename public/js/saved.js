    ...
    if (data.length > 0) {
      var $saves = data.map(function (artic) {
        ...
        var deleteButton = $('<button>')
          .addClass(
            'buttonMargin btn-small waves-effect waves-light right deleteIt'
          )
          .attr('type', 'submit')
          .attr('name', 'action')
        ...
        var noteButton = $('<button>')
          .addClass(
            'buttonMargin btn-small waves-effect waves-light modal-trigger right indigo addNote'
          )
          .attr('type', 'submit')
          .attr('name', 'action')
          .text('Add Note')
      ...
      var title = $('<h5>')
        .text('No Saved Articles. Go To The Home Page and Get Scraping!')
...
var addNote = function () {
  ...
  API.getSavedArticle(id).then(function (data) {
    ...
    var $row = $('<row>').append(
      $('<div>').addClass('col s10').append($('<h6>').text(data.note.body))
    );
    var button = $('<div>')
      .addClass('col s2')
      .append(
        $('<a>')
          .addClass('waves-effect waves-light right btn-floating')
          .attr('note-id', data.note._id)
          .attr('data-id', data._id)
          .attr('id', 'clearSavedNote')
          .append($('<i>').addClass('material-icons red').text('close'))
      );
    $row.append(button);
